...
- Release preparation now builds and tests from an isolated artifacts tree outside the repository, avoiding file-lock collisions with VS Code, DEV builds, test discovery, or other processes using the normal `bin/` and `obj/` folders.
- Reconciliation now distinguishes confirmed Windows file-lock owners from ownerless path-creation failures. GitPet only offers to close processes that Restart Manager confirms are holding affected paths; if absent paths remain blocked without an owner, it tells the user to save work and restart Windows, without prematurely recommending `chkdsk`.
- Standalone logical projects now expose **Reconcile ↕** when both the local project snapshot and its project-only online history changed. The action routes into the existing safe file-level standalone reconciliation workflow instead of leaving Get and Send disabled after a project switch.

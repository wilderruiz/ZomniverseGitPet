> Moved here from `docs/MAJOR_UPDATES_AND_RELEASES.md` as part of the Phase 4 canonical documentation pass. Content unchanged from the original — it remains accurate against current source (`MajorUpdateCoordinator.cs`, `MajorUpdateFeature.cs`, `MajorUpdateForm.cs`). See also [Update System](../developer/UPDATE_SYSTEM.md) for how this feature differs from GitPet's own self-update.

// Production renderer promoted from experiments/LynxLab/Direct2DTestControl.cs.
// Keep experiment and production changes deliberate; do not auto-link the files.
...
/// <summary>
/// Production Direct2D Guardian surface.
///
/// Promoted from the validated Lynx Lab Migration 5 renderer. PetForm owns
/// repository/activity semantics; this control owns native Direct2D drawing.
/// </summary>
internal sealed class PetDirect2DControl : Control
{
    ...
    private Color _canvasBackgroundColor =
        Color.FromArgb(0x0B, 0x10, 0x16);

    public PetDirect2DControl()
    {
        SetStyle(
        ...
        BackColor = _canvasBackgroundColor;
        TabStop = false;
    }

    public string BackendStatus => _status;
    public long FrameCount => _frameCount;
    public bool IsDirect2DReady =>
    ...
    private void RenderDirect2D()
    {
        ...
        var background =
            ToColorF(_canvasBackgroundColor);
        ...
        try
        {
            ...
            if (_frameCount == 1 || _frameCount % 120 == 0)
                SetStatus(
                    $"DIRECT2D GUARDIAN ✓ · {_frameCount} frames");
        }
        finally
        {
